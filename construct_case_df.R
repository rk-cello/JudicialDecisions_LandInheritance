...
#### acts_sections ####
acts_sections <- fread("/data/raw/acts_sections.csv")
...
for (year in year_list) {
  ...
  cases_year <- fread(paste0("/data/dev/cases/cases_", year, ".csv")) # read converted data instead of
  ...
  # save
  write.table(df_year, paste0("/data/dev/cases_merged/cases_", year, "_merged.csv"), 
              sep = ",", row.names = FALSE, col.names = TRUE, quote = TRUE)
  
  # save the initial inspection results to a text file
  capture.output(str(df_year), file = paste0("/txt/cases/inspection_cases_", year, ".txt"))
  capture.output(summary(df_year), file = paste0("/txt/cases/summary_cases_", year, ".txt"))  
